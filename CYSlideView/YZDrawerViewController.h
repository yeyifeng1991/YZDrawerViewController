...
@class YZDrawerViewController;
typedef NS_ENUM(NSInteger, YZDrawerViewControllerStyle) {
    YZDrawerViewControllerStyleNone, //抽屉菜单不滚动
   YZDrawerViewControllerStyleNormalSlide, // 抽屉菜单和中间的controller同步滚动
   YZDrawerViewControllerStyleParallaxSlide, // 抽屉菜单和中间的controller同时滚动, 但不同步
   YZDrawerViewControllerStyleScale // 中间的controller滚动时缩放
};

typedef NS_ENUM(NSInteger, YZDrawerViewControllerOpenStyle) {
    YZDrawerViewControllerOpenStyleFromScreenEdge, // 从屏幕边缘才能触发
    YZDrawerViewControllerStyleFromAnyWhere // 任何地方都可以触发
};

@protocol ZJDrawerControllerDelegate <NSObject>

- (BOOL)zj_drawerController:(YZDrawerViewController *)drawerController shouldBeginPanGesture: (UIPanGestureRecognizer *)panGesture;
...
@property (assign, nonatomic) YZDrawerViewControllerStyle drawerControllerStyle;
/** 打开抽屉手势触发的方式  --- 从屏幕边缘还是全屏 默认为ZJSlideControllerOpenStyleFromAnyWhere*/
@property (assign, nonatomic) YZDrawerViewControllerOpenStyle drawerControllerOpenStyle;
...
@end
@interface UIViewController (YZDrawerViewController)
/** 抽屉菜单 -- 控制器 */
@property (weak, readonly, nonatomic) YZDrawerViewController *yz_drawerController;
@end
